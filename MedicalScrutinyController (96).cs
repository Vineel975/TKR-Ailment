...
namespace Enrollment.Controllers
{
    ...
    public class MedicalScrutinyController : Controller
    {
        ...
        private string _claimLockKey = ConfigurationManager.AppSettings["ClaimLockKey"].ToString();
        private Int16 _claimLockTimeoutMinutes = Convert.ToInt16(ConfigurationManager.AppSettings["ClaimLockTimeoutMinutes"]);
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                if (Session[SessionValue.UserRegionID] != null && Convert.ToInt32(Session[SessionValue.UserRegionID]) != 0)
                {
                    ...
                    if (ClaimID != "" && SlNo != "" && SID != "")
                    {
                        ...
                        {
                            long claimId = Convert.ToInt64(ClaimID);
                            long userId = Convert.ToInt64(Session[SessionValue.LoginUserID]);
                            string userName = Session[SessionValue.LoginUserName].ToString();
                            if (MvcApplication.AWSCacheConnection != null && MvcApplication.AWSCacheConnection.IsConnected)
                            {
                                var db = MvcApplication.MemoryDB;
                                // STEP 1 : Check whether current claim is locked
                                RedisValue claimValue = db.HashGet(_claimLockKey, claimId.ToString());
                                if (claimValue.HasValue)
                                {
                                    var lockInfo = JsonConvert.DeserializeObject<ClaimLockModel>(claimValue);
                                    // Locked by another user
                                    if (lockInfo.UserId != userId)
                                    {
                                        ViewData["ErrorMsg"] = $"Claim locked by {lockInfo.UserName}";
                                        return View();
                                    }
                                }

                                // STEP 2 : User trying to lock another claim
                                foreach (HashEntry item in db.HashScan(_claimLockKey))
                                {
                                    var lockInfo = JsonConvert.DeserializeObject<ClaimLockModel>(item.Value);
                                    if (lockInfo.UserId == userId && lockInfo.ClaimId != claimId)
                                    {
                                        ViewData["ErrorMsg"] = "You have already locked another claim. Please unlock the previous claim.";
                                        return View();
                                    }
                                }

                                // STEP 3 : Create new lock
                                var newLock = new ClaimLockModel
                                {
                                    UserId = userId,
                                    ClaimId = claimId,
                                    UserName = userName,
                                ...
                                db.HashSet(_claimLockKey, claimId.ToString(), JsonConvert.SerializeObject(newLock));
                            }
            ...
            catch (Exception ex)
            {
                ...
                // "throw;" NOT "throw ex;".
                // throw ex RESETS the stack trace to this line, so every exception in this
                // controller reported itself as coming from here - the real origin was
                // discarded. That is what made the "no row at position 0" failure so hard
                // to place: the browser trace said line 270 while the fault was actually at
                // LoadBasicData line 370. (Elmah still had the original, because it logs ex
                // BEFORE the rethrow - that is how the real line was finally found.)
                // "throw;" preserves the original trace and changes nothing else.
                throw;
            }
        }
        ...
        private void LoadBasicData(string ClaimID, string SlNo, string SID, string QMS = "", bool isFrmArchived = false, string QMSadmin = "")
        {
            ...
            // AI Summary visibility is no longer resolved here.
            //
            // It used to call ResolveClaimAIMode, which ran USP_ClaimAI_GetResultStatus -
            // a second round trip for something USP_ClaimMedicalScrutiny_Retrieve can
            // return in the same query it already makes. That procedure now LEFT JOINs
            // ClaimAI_Results (via OUTER APPLY on the latest row) and exposes two columns:
            //
            //     ClaimAI_ProcessingStatus
            //     ClaimAI_JobId
            //
            // They travel to the page inside ViewData["BasicData"], which is the whole
            // dtBasicData serialised, so the view reads them from basicData[0] with no
            // extra call. ResolveClaimAIMode has been removed entirely.
            ViewData["QMS"] = QMS;
            ...
            // GUARDED. This was the ONLY unguarded .Rows[0] in LoadBasicData - every other
            // access here is wrapped in a Rows.Count > 0 check - and it threw
            // "There is no row at position 0" when opening a claim, before any of the
            // claim data further down had even been read.
            //
            // GetIsClaimCopay is keyed on the LOGGED-IN USER, not the claim, so an empty
            // result means that user has no copay row configured. That is a missing-
            // configuration case, not a broken claim: default to "0" (no copay) so the
            // claim still opens, and log it so the gap stays visible.
            if (IsCopay != null && IsCopay.Rows.Count > 0)
            {
            ...
            {
                ViewData["IsCopay"] = "0";
                TariffLog("[ClaimAI] GetIsClaimCopay returned no rows for user "
                    + Convert.ToString(Session[SessionValue.LoginUserID])
                    + " (claim " + ClaimID + "/" + SlNo + "). Defaulting IsCopay to 0.");
            }
            ...
            // GUARDED. These four were unguarded .Rows[0] accesses and threw
            // "There is no row at position 0" from LoadBasicData line 370, which is the
            // real origin of the claim-open failure. Note the line directly ABOVE them
            // does check dtbhimavalidationsData.Rows.Count > 0 - these four were simply
            // missed.
            //
            // They come from CheckBoxDetails_DataVM .. CheckBoxDetails3_DataVM. An empty
            // result means the claim has no saved value for that checkbox yet, which is a
            // normal state for a claim that has not been through this screen - not an
            // error. Default to DBNull so the view binds exactly as it did when the row
            // was absent-but-empty, rather than failing the whole page.
            ViewData["dtRadio1data"] = dtRadio1data != null && dtRadio1data.Rows.Count > 0
                ? dtRadio1data.Rows[0]["Radio1"] : (object)DBNull.Value;
            ViewData["dtRadio2data"] = dtRadio2data != null && dtRadio2data.Rows.Count > 0
                ? dtRadio2data.Rows[0]["Radio2"] : (object)DBNull.Value;
            ViewData["dtRadio3data"] = dtRadio3data != null && dtRadio3data.Rows.Count > 0
                ? dtRadio3data.Rows[0]["Radio3"] : (object)DBNull.Value;
            ViewData["dtRadio4data"] = dtRadio4data != null && dtRadio4data.Rows.Count > 0
                ? dtRadio4data.Rows[0]["Radio4"] : (object)DBNull.Value;

            // One line naming which of the four was empty - the SPs are keyed on the
            // claim, so an empty result points at missing claim data, not a code fault.
            if ((dtRadio1data?.Rows.Count ?? 0) == 0 || (dtRadio2data?.Rows.Count ?? 0) == 0
             || (dtRadio3data?.Rows.Count ?? 0) == 0 || (dtRadio4data?.Rows.Count ?? 0) == 0)
            {
                TariffLog("[ClaimAI] CheckBoxDetails empty for claim " + ClaimID + "/" + SlNo
                    + " - Radio1:" + (dtRadio1data?.Rows.Count ?? 0)
                    + " Radio2:" + (dtRadio2data?.Rows.Count ?? 0)
                    + " Radio3:" + (dtRadio3data?.Rows.Count ?? 0)
                    + " Radio4:" + (dtRadio4data?.Rows.Count ?? 0));
            }
            //SP3V-1623
            ...
            ViewData["LastSlNo"] = _objMadicalScrutinyVM.GetLastExtensionNumber(Convert.ToInt64(ClaimID), isFrmArchived);
            ...
            if (dtBasicData.Rows.Count > 0)
            {
                ...
                if (dtBasicData.Rows[0]["healthpaystatus"].ToString() == "")
                {
                    ViewData["healthpaystatus"] = "";
                }
                else
                {
                    ViewData["healthpaystatus"] = dtBasicData.Rows[0]["healthpaystatus"].ToString();
                }
                ...
                ViewData["ServiceTypeID"] = dtBasicData.Rows[0]["ServiceTypeID"];
                ...
                ViewData["MEMBERSIID"] = dtBasicData.Rows[0]["MEMBERSIID"];
                ViewData["SICategoryID"] = dtBasicData.Rows[0]["SICategoryID"];
        ...
        public string ClaimRules_Insert(string ClaimDetails, string Rules, Decimal DiscountByHospital, Decimal EligibleAmount, Decimal Deductible, Decimal CoPayment,
            ...
            string QMSID, string QMSAdminID, Decimal Modularamount, Decimal Patienttobepaid, bool Adj_IsFinal, string Isrefertocrm, string Isrefertoinsurer, bool SkipAudit, Decimal? PMTNegotiatedDiscount,
            Decimal? NotCollectFromInsured, string SkipMedicalScrutinyReasons, string SkipMedicalScrutinyRemarks)
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    _objMadicalScrutinyVM.ClaimRules_Insert(objActionIteams, rules, DiscountByHospital, EligibleAmount, Deductible, CoPayment, NetEligibleAmount
                        , Excess_SI, Excess_Preauth, ExcessPaidByPatient, NottoDeductFromHospital, AdmissibleAmount
                        , EligiblePayableAmount, NegotiatedAmount, GrossAmount, TDSAmount, NetAmount
                        , PaidByPatient, Copayhtml, dtUtilization, DoctorNotes, AdditionalNotes, BufferUtilized
                        , EarlyPaymentDiscountAmount, SkipScrutiny, PremiumDeducted, Modularamount, Patienttobepaid
                        , Adj_IsFinal, Isrefertocrm, Isrefertoinsurer, SkipAudit, PMTNegotiatedDiscount
                        , NotCollectFromInsured, SkipMedicalScrutinyReasons, SkipMedicalScrutinyRemarks, out msg);
        ...
        public string Adjudication_Actions_Insert(string ClaimDetails, string QMSID, string QMSAdminID, string ClaimRejections, string ddlInvestigationReasons = ""
            ...
            , string txtSuspect_Fraudster_Name = "", string ddlSuspect_Fraudster_Proof_ID = "", string txtSuspect_Fraudster_ID_Proof_Number = "", string txtfieldofcname = "", string InvestigationModeType = "", string RTI_deductible = "0", bool Is_final = false)
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    dsResult = _objMadicalScrutinyVM.Adjudication_Actions_Insert(objActionIteams, dtClaimRejections, out msg, ddlInvestigationReasons
                        ...
                        , ddlSuspect_Fraudster_Proof_ID, txtSuspect_Fraudster_ID_Proof_Number, officer, InvestigationModeType, Is_final);
                    ...
                    if (dsResult.Tables.Count != 0)
                    {
                        if (dsResult.Tables.Count != 0)
                        {
                            ...
                            else if (dsResult.Tables[0].Rows.Count > 0 && dsResult.Tables[1].Rows.Count > 0 && Convert.ToInt32(dsResult.Tables[1].Rows[0]["IssueId"].ToString()) == 26 && objActionIteams.ReasonIDs_P == "224" && Convert.ToInt32(objActionIteams.ClaimStageID) == 14)
                            {
                                _objCommon.CommunicationInsert_Common(ref dsResult, Convert.ToInt64(JObject["ClaimID"]), Convert.ToInt32(JObject["Slno"]), Convert.ToInt64(dsResult.Tables[1].Rows[0]["MainMemberPolicyID"].ToString()),
                                         Convert.ToInt64(dsResult.Tables[1].Rows[0]["PolicyID"].ToString()), Convert.ToInt64(dsResult.Tables[1].Rows[0]["ProviderID"].ToString()), Convert.ToInt32(dsResult.Tables[1].Rows[0]["BrokerID"].ToString()), Convert.ToInt64(dsResult.Tables[1].Rows[0]["CorporateID"].ToString()),
                                         Convert.ToInt64(dsResult.Tables[1].Rows[0]["PayerID"].ToString()), Convert.ToInt32(dsResult.Tables[1].Rows[0]["Issueid"].ToString()), 24, "MedicalScrutinyController",
                                         Convert.ToInt32(Session[SessionValue.UserRegionID]), 0, Convert.ToInt32(dsResult.Tables[1].Rows[0]["Agentcode"].ToString() == "" ? "0" : dsResult.Tables[1].Rows[0]["Agentcode"].ToString()));

                            }
                            else if (dsResult.Tables[0].Rows.Count > 0 && dsResult.Tables[1].Rows.Count > 0 && (objActionIteams.ReasonIDs_P == "720" || objActionIteams.ReasonIDs_P == "721") && Convert.ToInt32(objActionIteams.ClaimStageID) == 10)
                            {
                                _objCommon.CommunicationInsert_Common(ref dsResult, Convert.ToInt64(JObject["ClaimID"]), Convert.ToInt32(JObject["Slno"]), Convert.ToInt64(dsResult.Tables[1].Rows[0]["MainMemberPolicyID"].ToString()),
                                         Convert.ToInt64(dsResult.Tables[1].Rows[0]["PolicyID"].ToString()), Convert.ToInt64(dsResult.Tables[1].Rows[0]["ProviderID"].ToString()), Convert.ToInt32(dsResult.Tables[1].Rows[0]["BrokerID"].ToString()), Convert.ToInt64(dsResult.Tables[1].Rows[0]["CorporateID"].ToString()),
                                         Convert.ToInt64(dsResult.Tables[1].Rows[0]["PayerID"].ToString()), Convert.ToInt32(dsResult.Tables[1].Rows[0]["Issueid"].ToString()), 24, "MedicalScrutinyController",
                    ...
                    string msg1 = "";
                    DataTable Data_refer_to_insurer = _objMadicalScrutinyVM.Check_Insurer_Refer_to_insurer(Convert.ToInt64(objActionIteams.ClaimID), Convert.ToInt32(objActionIteams.Slno), 2, objActionIteams.ReasonIDs_P);
                    if (Data_refer_to_insurer.Rows.Count > 0 && Data_refer_to_insurer.Rows[0]["Isrefertoinsurer"].ToString() == "True" && msg.Contains("Saved Successfully"))
                    {
                        if (objActionIteams.ReasonIDs_P == "224" && Convert.ToInt32(objActionIteams.ClaimStageID) == 14 && Data_refer_to_insurer.Rows[0]["RTI_reasonID"].ToString() == "220")
                        {
                        ...
                        else if (objActionIteams.ReasonIDs_P == "225" && Convert.ToInt32(objActionIteams.ClaimStageID) == 14 && Data_refer_to_insurer.Rows[0]["RTI_reasonID"].ToString() == "221")
                        {
                            dtresult1 = _objMadicalScrutinyVM.USP_ClaimAudit_Insert_Refer_to_Insurer(objActionIteams, out msg1);
                            if (dtresult1.Tables.Count != 0)
                            {
                    ...
                    if (IssueId == 12 && (Convert.ToInt32(objActionIteams.ClaimStageID) == 17 || Convert.ToInt32(objActionIteams.ClaimStageID) == 7) && objActionIteams.RequestTypeID < 4)
                    {
                        msg = RSAI_RefertoInsurerAtForAuditStage(Convert.ToInt64(objActionIteams.ClaimID), Convert.ToInt32(objActionIteams.Slno));
                    }
            ...
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    if (IssueId != 9)
                    {
                        ...
                        DataTable is_refer_to_insurer = _objMadicalScrutinyVM.Check_Insurer_Refer_to_insurer(Convert.ToInt64(objActionIteams.ClaimID), Convert.ToInt32(objActionIteams.Slno), 1);
                        bool isReferToInsurer = (is_refer_to_insurer?.Rows.Count > 0 && is_refer_to_insurer.Rows[0]["Isrefertoinsurer"].ToString() == "True") ? true : false;
                        if ((IssueId == 31 || (!isRestrictedITCIPolicyNo && IssueId == 10 && objActionIteams.RequestTypeID != 1 && objActionIteams.RequestTypeID != 2 && objActionIteams.RequestTypeID != 3) || isReferToInsurer) && isApprove)//GoDigit
                        {
                            ...
                            else if (IssueId == 12 && (Convert.ToInt32(objActionIteams.ClaimStageID) == 17 || Convert.ToInt32(objActionIteams.ClaimStageID) == 7))
                            {
                                msg = RSAI_RefertoInsurerAtForAuditStage(Convert.ToInt64(objActionIteams.ClaimID), Convert.ToInt32(objActionIteams.Slno));
                            }
                            else
            ...
            string QMSID, string QMSAdminID)
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    if (Convert.ToInt32(InsuranceCompanyID) == 12 && Convert.ToInt32(RequestTypeID) < 4)
                    {
                        RSAI_RefertoInsurerAtForAuditStage(Convert.ToInt64(ClaimID), Convert.ToInt32(Slno));
                    }
        ...
        public string SaveCalculationReProcessInfo(string ClaimDetails, string Rules, Decimal DiscountByHospital, Decimal EligibleAmount, Decimal Deductible, Decimal CoPayment,
           ...
           , Decimal Modularamount, Decimal Patienttobepaid, Decimal? PMTNegotiatedDiscount, Decimal? NotCollectFromInsured)
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    var dt = _objMadicalScrutinyVM.SaveCalculationReProcessInfo(
                        ...
                        AdditionalNotes, BufferUtilized, rules, EarlyPaymentDiscountAmount, AuditSkipScrutiny,
                        PremiumAmount, Modularamount, Patienttobepaid, PMTNegotiatedDiscount, NotCollectFromInsured);
        ...
        public string RSAI_RefertoInsurerAtForAuditStage(long ClaimID, int slno)
        {
            string msg = string.Empty;
            try
            {
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12;
                var insurerresobj = new RSAIRTAAPIResponse();
                string token = Convert.ToString(ConfigurationManager.AppSettings["RTIRoyalSundaramToken"]);
                var client = new HttpClient();
                string apiUrl = Convert.ToString(ConfigurationManager.AppSettings["RTIRoyalSundaramURL"]);
                RSAIRTAAPIRequest docReq = new RSAIRTAAPIRequest();
                docReq.claimID = ClaimID;
                docReq.slno = slno;
                var jsonDoc = JsonConvert.SerializeObject(docReq).ToString();
                var request = new HttpRequestMessage(HttpMethod.Post, apiUrl);
                ...
                var response = client.SendAsync(request);
                var insurerresponse = response.Result.Content.ReadAsStringAsync().Result;
                insurerresobj = JsonConvert.DeserializeObject<RSAIRTAAPIResponse>(insurerresponse);
                msg = "Status " + " " + insurerresobj.status + " " + insurerresobj?.message;
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog errorLog = Elmah.ErrorLog.GetDefault(null);
                errorLog.ApplicationName = System.Web.Configuration.WebConfigurationManager.AppSettings["AppName"].ToString();
                errorLog.Log(new Elmah.Error(ex));
                msg = "RSAI Status Code" + " " + 500 + " " + "Internal server, please contact admin";
            }
            return msg;
        }
        ...
        //Added for Spectra-iAI integration (SP3V-4924)
        public async Task<JsonResult> LockiAIProcessClaim(string ClaimID)
        {
            List<object> objRes = new List<object>();
            if (Session[SessionValue.UserRegionID] != null)
            {
                ...
                if (claimValue.HasValue)
                {
                    ...
                    if (lockInfo.UserId != userId)
                    {
                        objRes.Add(new { Result = false, ResponseText = $"Claim locked by {lockInfo.UserName}" });
                    }
                }
                var checkIsAssigend = false;
                // STEP 2 : User trying to lock another claim
                foreach (HashEntry item in db.HashScan(_claimLockKey))
                {
                    var lockInfo = JsonConvert.DeserializeObject<ClaimLockModel>(item.Value);
                    if (lockInfo.UserId == userId && lockInfo.ClaimId != claimId)
                    {
                        objRes.Add(new { Result = false, ResponseText = $"You have already locked another claim. Please unlock the previous claim." });
                        checkIsAssigend = true;
                    }
                }
                if (!checkIsAssigend)
                {
            ...
            var filter = new UsersAuditlogFilter();
            await filter.UpdateCacheUserActionAsync(Convert.ToInt32(Session[SessionValue.LoginUserID]), User.Identity.Name, "MedicalScrutiny", "IAI Process");
            return jsonResult;
        }
        ...
        public ActionResult StartClaimAuditProxy()
        {
            ...
            try
            {
                ...
                // GIPSA PPN->SOC two-pass: fetch the SOC fallback file server-side.
                // Returns empty unless PSU + GIPSA zip has both PPN and SOC (guarded
                // inside GetTariffDocument). Mirrors the staging path so BOTH the
                // browser and staging flows get the two-pass.
                string socFileName = null; byte[] socBytes = null;
                try
                {
                    int slForSoc = 1;
                    long claimIdForSoc;
                    if (long.TryParse((claimId ?? "").Trim(), out claimIdForSoc))
                    {
                        string connStrSoc = System.Configuration.ConfigurationManager
                            .ConnectionStrings["McarePlusEntities"].ConnectionString;
                        if (connStrSoc.StartsWith("metadata=", StringComparison.OrdinalIgnoreCase))
                        {
                            var mSoc = System.Text.RegularExpressions.Regex.Match(
                                connStrSoc, "provider connection string=\\\"([^\\\"]+)\\\"",
                                System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                            if (mSoc.Success) connStrSoc = mSoc.Groups[1].Value.Replace("&quot;", "\"");
                        }
                        using (var connSoc = new System.Data.SqlClient.SqlConnection(connStrSoc))
                        {
                            connSoc.Open();
                            var cmdSoc = connSoc.CreateCommand();
                            cmdSoc.CommandTimeout = 300;
                            cmdSoc.CommandType = System.Data.CommandType.StoredProcedure;
                            cmdSoc.CommandText = "USP_ClaimAI_ResolveClaimRow";
                            cmdSoc.Parameters.AddWithValue("@ClaimID", claimIdForSoc);
                            cmdSoc.Parameters.Add("@SlNo", System.Data.SqlDbType.Int).Value = DBNull.Value;
                            using (var rdrSoc = cmdSoc.ExecuteReader())
                            {
                                if (rdrSoc.Read() && rdrSoc["LowestSlNo"] != DBNull.Value)
                                    slForSoc = Convert.ToInt32(rdrSoc["LowestSlNo"]);
                            }
                        }
                    }
                    string socB64 = null, socPageCats = null;
                    ExtractDocBase64(GetTariffDocument(claimId, slForSoc.ToString(), true),
                        out socB64, out socFileName, out socPageCats);
                    if (!string.IsNullOrWhiteSpace(socB64))
                    {
                        try { socBytes = Convert.FromBase64String(socB64); } catch { socBytes = null; }
                    }
                }
                catch { /* SOC optional */ }
                ...
                // Add tariff + soc file names to spectraFields so ClaimAI can tag the
                // extraction with whichever file the rows came from.
                if (!string.IsNullOrWhiteSpace(spectraFieldsJson))
                {
                    try
                    {
                        var _sfB = Newtonsoft.Json.Linq.JObject.Parse(spectraFieldsJson);
                        if (!string.IsNullOrWhiteSpace(tariffFileName)) _sfB["tariffFileName"] = tariffFileName;
                        if (!string.IsNullOrWhiteSpace(socFileName)) _sfB["socTariffFileName"] = socFileName;
                        spectraFieldsJson = _sfB.ToString(Newtonsoft.Json.Formatting.None);
                    }
                    catch { /* leave spectraFieldsJson as-is on parse failure */ }
                }
        ...
        /// <summary>
        /// STAGING STEP 1 — Disease classifier.
        ///
        /// Determines whether a claim is a disease type that ClaimAI supports
        /// for pre-processing (currently cataract and maternity). The staging
        /// worker uses this to decide: supported -> process via ClaimAI; else
        /// -> move straight to "Ready for Adjudication" (StageID 5).
        ///
        ...
        ///   - anything else                         -> "other"
        ///
        /// To support a new disease later, add its Level1 specialty value(s) to
        /// the relevant set below — no other code change needed.
        ///
        /// Returns "cataract", "maternity", or "other". On any error or missing
        /// data, returns "other" (safe default — claim goes straight to stage 5
        ...
        /// <summary>
        /// True when the claim is a reimbursement claim (ClaimTypeID=2 or
        /// RequestTypeID=4), mirroring the IsClaimAISummaryAllowed source. Tells
        /// ClaimAI to run the reimbursement document checklist during staging.
        /// Never throws — defaults to false.
        /// </summary>
            ...
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    using (var cmd = new System.Data.SqlClient.SqlCommand(
                        "SELECT TOP 1 ClaimTypeID, RequestTypeID FROM Claimsdetails WITH(NOLOCK) WHERE ClaimID=@cid AND ISNULL(Deleted,0)=0 ORDER BY SlNo DESC", conn))
                    {
                        cmd.CommandTimeout = 300;
        ...
        /// <summary>
        /// Staging fallback classifier. When the procedure-based
        /// GetClaimDiseaseTypeForStaging cannot determine a disease (e.g. a claim
        /// at StageID 52 has no medical-scrutiny coding yet, so level3_ID is
        /// unset), classify from the recorded diagnosis text via ClaimAI — the
        /// same call the on-demand GetClaimType endpoint makes.
        /// Returns "cataract" | "maternity" | "other"; never throws.
        /// </summary>
        ...
        /// <summary>
        /// Classifies a diagnosis string via ClaimAI. Split out of
        /// ClassifyDiseaseByDiagnosisTextForStaging so a caller that already HAS the
        /// text - which the staging path now does - does not pay for a second
        /// database round trip to fetch it again.
        /// Returns "cataract" | "maternity" | "tkr" | "other"; never throws.
        /// </summary>
        ...
        /// <summary>
        /// Recursively walks a zip stream and collects every leaf tariff file
        /// (.pdf/.xlsx/.xls) as (displayName, lastModified, bytes).
        ///
        /// The displayName carries the FULL CHAIN of containing inner-zip names
        /// (with the ".zip" extension stripped), joined by "_", so the AI can
        /// see which zip a file came from. The OUTERMOST zip (the root
        /// "tariff.zip") is NOT part of the prefix — only inner zips are.
        ///
        /// Examples (root = tariff.zip):
        ...
        ///
        /// prefix: cumulative prefix for the current zip level ("" at root).
        /// </summary>
        ...
        /// <summary>
        /// GIPSA PPN->SOC two-pass helper. Returns TRUE when the candidate set (from the
        /// GIPSA zip) contains BOTH a PPN file (GIPSA, not SOC) AND a non-PPN/SOC file
        /// (GIPSA SOC) — the precondition for sending both files to ClaimAI.
        /// </summary>
        ...
        /// <summary>
        /// GIPSA PPN->SOC two-pass helper. Picks the GIPSA SOC (non-PPN) file from the
        /// candidate set — the fallback tariff. Mirrors PickBestTariffFile's P3 tier
        /// (filename contains "gipsa" AND "soc"), latest-modified first, converted to PDF.
        /// Returns null when no SOC file exists.
        /// </summary>
        ...
        // REMOVED: GetClaimType. Claim type now comes from ClaimAI_Results.DiseaseType
        // via basicData, so the page no longer derives it from the diagnosis on load.
            ...
            try
            {
                ...
                foreach (var claimEntry in claimBatch)
                {
                    ...
                    try
                    {
                        ...
                        // Phase 2a auto-save inputs (captured from the decision when eligible).
                        bool autoDoSave = false;
                        string autoApprovedAmount = null, autoApprovalBasis = null;
                        string autoHospEstimate = null, autoTariffAmount = null;
                        try
                        {
                            if (!string.IsNullOrWhiteSpace(analysisJson))
                            {
                                ...
                                if (_aa != null)
                                {
                                    ...
                                    // Capture the amounts needed for the auto-save when eligible.
                                    autoApprovalBasis = (string)(_aa.approvalBasis ?? null);
                                    autoApprovedAmount = _aa.approvedAmount != null ? (string)_aa.approvedAmount.ToString() : null;
                                    // Hospital estimate + tariff total for the save payload
                                    // (baseInsurerPayable is the after-discount hospital figure).
                                    try { if (_an.baseInsurerPayable != null) autoHospEstimate = (string)_an.baseInsurerPayable.ToString(); } catch { }
                                    try
                                    {
                                        if (_an.tariffExtractionItem != null)
                                        {
                                            ...
                                            if (_tsum > 0) autoTariffAmount = _tsum.ToString();
                                        }
                                    }
                                    catch { }
                                    // The SAVE (billing/clinical/coding/hosp details) should run
                                    // for EVERY claim that has save data — approve, refer_crm, or
                                    // query alike. It is decoupled from autoApproved, which governs
                                    // only the (Phase 2b-2d) money action. So gate the save on the
                                    // presence of an approval basis + approved amount, not eligibility.
                                    autoDoSave = !string.IsNullOrWhiteSpace((string)(_aa.approvalBasis ?? null))
                                                 && _aa.approvedAmount != null;
                        ...
                        // ── PHASE 2a: session-less auto-Save for eligible claims ──────
                        // Gated by config: only runs when EnableAutoApprovalSave=true AND
                        // the decision was eligible. This performs the doctor's iframe
                        // "Save" (approved amount + basis) with the configured system
                        // identity. The subsequent actions (copay/calculate, query, CRM,
                        // final approve) are Phase 2b–2d, added incrementally.
                        bool autoSaveEnabled = string.Equals(
                            (System.Configuration.ConfigurationManager.AppSettings["EnableAutoApprovalSave"] ?? "false").Trim(),
                            "true", StringComparison.OrdinalIgnoreCase);
                        bool autoSaved = false;
                        string autoSaveStatus = null, autoSaveError = null;
                        if (autoSaveEnabled && autoDoSave && !string.IsNullOrWhiteSpace(autoApprovedAmount))
                        {
                            autoSaved = AutoPerformFullSaveForStaging(
                                claimId, slNo, disease, analysisJson,
                                autoHospEstimate, autoTariffAmount, autoApprovedAmount, autoApprovalBasis,
                                out autoSaveStatus, out autoSaveError);
                            // Record the outcome so a partial/failed save (after the stage
                            // already flipped to 5) is queryable — does not change the stage.
                            RecordAutoSaveResult(connStr, claimId, slNo, autoSaveStatus, autoSaveError);
                        }
                        else if (autoDoSave && !autoSaveEnabled)
                        {
                            // Has save data but the save is disabled by config — mark skipped.
                            RecordAutoSaveResult(connStr, claimId, slNo, "skipped", "EnableAutoApprovalSave=false");
                        }

                        processed++;
                        details.Add(new { claimId, disease, status = "done", jobId, autoApproved, autoReason, autoSaved, autoSaveStatus });
                    }
                        ...
                        if (_docsMissing)
                        {
                            try
                            {
                                using (var _ac = new System.Data.SqlClient.SqlConnection(connStr))
                                {
                                    ...
                                    _cmd.CommandText =
                                        @"SELECT TOP 1 ISNULL(AttemptCount, 0)
                                          FROM dbo.ClaimAI_Results WITH (NOLOCK)
                                          WHERE ClaimID = @cid ORDER BY ID DESC";
                                    _cmd.Parameters.AddWithValue("@cid", claimId);
            ...
            using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
            {
                ...
                cmd.CommandText = @"
                    ...
                    WHEN MATCHED THEN UPDATE SET
                        ...
                        LastError        = @LastError,
                        AutoApproved     = COALESCE(@AutoApproved, t.AutoApproved),
                        BillingStateJson = COALESCE(@BillingStateJson, t.BillingStateJson),
        ...
        private void SetClaimStage(string connStr, long claimId, int stageId)
        {
            using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
            {
                ...
                // Claims table — the canonical claim stage.
                var cmd = conn.CreateCommand();
                cmd.CommandTimeout = 300;
                ...
                // claimActionItems: keep a row per stage instead of editing one row in
                // place. The open row (stage 52) is CLOSED with CloseDate = now, and a
                // NEW row is inserted for the target stage with OpenDate = the same
                // timestamp. Every other column is copied verbatim, so the pair differs
                // only in ClaimStageID / OpenDate / CloseDate.
                //
                // The column list is discovered from INFORMATION_SCHEMA rather than
                // written out here: claimActionItems has more columns than the
                // ClaimActionItems model exposes, and a hard-coded list would silently
                // drop any column added later. Identity and computed columns are excluded.
...
-- The row currently open for this claim. UPDLOCK/HOLDLOCK so a concurrent stage
-- change cannot pick the same row between the SELECT and the UPDATE below.
DECLARE @srcId bigint = (
    SELECT TOP 1 ID
    FROM dbo.claimActionItems WITH (UPDLOCK, HOLDLOCK)
    WHERE ClaimID = @cid AND CloseDate IS NULL
    ORDER BY OpenDate DESC, ID DESC);

-- Nothing to do when there is no open row, or it is already at the target stage
-- (guards against inserting a duplicate if this is ever called twice).
IF @srcId IS NOT NULL
   AND (SELECT ClaimStageID FROM dbo.claimActionItems WHERE ID = @srcId) <> @stage
BEGIN
    ...
    SELECT @cols = STUFF((
        ...
        WHERE c.TABLE_NAME = 'claimActionItems'
          AND COLUMNPROPERTY(OBJECT_ID('dbo.claimActionItems'), c.COLUMN_NAME, 'IsIdentity') = 0
          AND COLUMNPROPERTY(OBJECT_ID('dbo.claimActionItems'), c.COLUMN_NAME, 'IsComputed') = 0
        ORDER BY c.ORDINAL_POSITION
        FOR XML PATH(''), TYPE).value('.', 'nvarchar(max)'), 1, 1, '');

    SELECT @sel = STUFF((
        SELECT ',' + CASE c.COLUMN_NAME
                        WHEN 'ClaimStageID' THEN '@s'
                        WHEN 'OpenDate'     THEN '@n'
    ...
    SET @sql = N'INSERT INTO dbo.claimActionItems (' + @cols + N')
                 SELECT ' + @sel + N'
                 FROM dbo.claimActionItems WHERE ID = @id;';

    EXEC sp_executesql @sql,
         N'@id bigint, @s int, @n datetime',
         @id = @srcId, @s = @stage, @n = @now;

    UPDATE dbo.claimActionItems SET CloseDate = @now WHERE ID = @srcId;
END

COMMIT;";
                cmd2.Parameters.AddWithValue("@stage", stageId);
                cmd2.Parameters.AddWithValue("@cid", claimId);
        ...
        public ActionResult GetStagingResultForClaim(string claimId)
        {
            try
            {
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    var cmd = conn.CreateCommand();
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.CommandText = "USP_ClaimAI_GetResultStatus";
                    cmd.Parameters.AddWithValue("@ClaimID", cId);
                    // NULL = latest row across all SlNos, as this query always did.
        ...
        /// <summary>
        /// Records the result of an ON-DEMAND run so the claim is not re-processed on
        /// every open. Called from the browser after a successful Save.
        ///
        /// Only fills a GAP: if the row is already 'done' with a jobId (a proper staged
        /// result) it is left completely alone, so a staged claim that a reviewer saves
        /// again keeps its original staging record. Anything else - failed, skipped,
        /// processing, or no row at all - is upserted to 'done' with this jobId.
        /// </summary>
        [HttpPost]
        [AllowAnonymous]
        [OverrideAuthorization]
        public JsonResult StoreOnDemandResultForClaimAI(string claimId, string slNo, string jobId)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(claimId) || string.IsNullOrWhiteSpace(jobId))
                    return Json(new { Success = false, Message = "claimId and jobId are required" });

                if (!long.TryParse(claimId.Trim(), out long cId))
                    return Json(new { Success = false, Message = "bad claimId" });
                int sNo;
                if (!int.TryParse((slNo ?? "1").Trim(), out sNo) || sNo <= 0) sNo = 1;

                string connStr = GetStagingConnString();
                if (string.IsNullOrWhiteSpace(connStr))
                    return Json(new { Success = false, Message = "no connection string" });

                string existingStatus = null, existingJobId = null, diseaseType = null;
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    ...
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.CommandText = "USP_ClaimAI_GetResultStatus";
                    // This one DOES filter by SlNo - kept as-is. See the doc: it is why
                    // this method and ResolveClaimAIMode can disagree about a claim that
                    // has rows for more than one SlNo.
                    cmd.Parameters.AddWithValue("@ClaimID", cId);
                    ...
                    using (var rdr = cmd.ExecuteReader())
                    {
                        if (rdr.Read())
                        {
                            existingStatus = rdr["ProcessingStatus"] == DBNull.Value ? null : rdr["ProcessingStatus"].ToString();
                            existingJobId = rdr["ClaimAI_JobId"] == DBNull.Value ? null : rdr["ClaimAI_JobId"].ToString();
                            diseaseType = rdr["DiseaseType"] == DBNull.Value ? null : rdr["DiseaseType"].ToString();
                        }
                    }
                }

                // Already a good staged result - do not touch it.
                if (string.Equals((existingStatus ?? "").Trim(), "done", StringComparison.OrdinalIgnoreCase)
                    && !string.IsNullOrWhiteSpace(existingJobId))
                {
                    TariffLog("[ClaimAI][OnDemand] claim " + cId + " already 'done' with jobId "
                        + existingJobId + " - left unchanged.");
                    return Json(new { Success = true, Message = "already done", Stored = false });
                }

                // DiseaseType is passed through so an existing value survives; when the row
                // is new it stays null, which nothing in the gate depends on.
                UpsertStagingResult(connStr, cId, sNo, diseaseType, "done", jobId.Trim(),
                                    null, null, null, null);

                TariffLog("[ClaimAI][OnDemand] claim " + cId + " stored as done, jobId " + jobId.Trim()
                    + " (previous status: " + (existingStatus ?? "no row") + ")");
                return Json(new { Success = true, Stored = true });
            }
            catch (Exception ex)
            {
                // Never fail the user's save because of this - it is an optimisation.
                TariffLog("[ClaimAI][OnDemand] store failed for claim " + claimId + ": " + ex.Message);
                return Json(new { Success = false, Message = ex.Message });
            }
        }

        // REMOVED: ResolveClaimAIMode. The AI Summary state now comes from
        // USP_ClaimMedicalScrutiny_Retrieve's OUTER APPLY on ClaimAI_Results, so this
        // second stored-procedure call is no longer made.
        ...
        /// </summary>
        /// <summary>
        /// Hardcoded test claim IDs for which medical bill + tariff should be
        /// loaded from the local /ClaimAIDocs folder, even in prod/preprod.
        /// All other claims use the normal DMS/S3 flow based on Web.config Enviroment key.
        /// REMOVE THIS LIST once testing is done.
        /// </summary>
        private static readonly System.Collections.Generic.HashSet<string> _HardcodedTestClaimIds =
            new System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "26051796629", "26051796637", "26051796654", "26051796671", "26051796762",
            "26051796423", "26051796577", "26051796489", "26051796501", "26051796596",
            "26051591856", "26051694326", "26051897945", "26051591854", "26051590845",
            "26051591486", "26051696018", "26051897809", "26051485174", "26051591399",
            "26051695562", "26051696051", "26051898316", "26051899125", "26051899333",
            "26051899932", "26051591758", "26051593677", "26051694447", "26051591708"
        };

        private static bool IsHardcodedTestClaim(string claimId)
        {
            return !string.IsNullOrWhiteSpace(claimId) &&
                   _HardcodedTestClaimIds.Contains(claimId.Trim());
        }
        ...
        public ActionResult GetMedicalBillDocument(string claimId = null, string slNo = null)
        {
            ...
            try
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("[ClaimAI] Test claim override — using LOCAL file path for claimId=" + cId);
        ...
        // Wrap a raster image (jpg/png/gif/bmp/tiff) into a single-page PDF sized
        // to the image, so image documents merge and RENDER alongside PDFs.
        ...
        // Like MergePdfs, but records the DMS category of each ACTUAL merged page
        // (after duplicate-page removal) so ClaimAI's sub-tabs line up exactly with
        // the rendered bill. categories[i] is the category of pdfList[i].
        ...
        public ActionResult SaveBillingForClaimAI(
            ...
            string totalAmountApproved = null, string approvalBasis = null)
        {
            try
            {
                if (Session[SessionValue.UserRegionID] == null)
                    return Json(new { success = false, message = "Session expired" });
                if (Convert.ToString(Session["ClaimStageID"]) != "5")
                    return Json(new { success = false, message = "AI Summary is read-only outside the Adjudication stage." });
                ...
                // Session identity for the acting adjudicator (browser path).
                int sUserRegionId = Convert.ToInt32(Session[SessionValue.UserRegionID]);
                int sRegionId = Convert.ToInt32(Session[SessionValue.RegionID]);
                int sRoleId = Session[SessionValue.UserRoleID] != null
                                       ? Convert.ToInt32(Session[SessionValue.UserRoleID])
                                       : 20;
                System.Data.DataTable sDisc = (System.Data.DataTable)(Session["ClaimDiscount"]);

                string coreMsg;
                bool coreOk = SaveBillingForClaimAICore(
                    claimIdLong, slNoInt,
                    hospitalBillAmount, tariffAmount, totalAmountApproved, approvalBasis,
                    sUserRegionId, sRegionId, sRoleId, sDisc, out coreMsg);
                return Json(new { success = coreOk, message = coreMsg });
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog.GetDefault(null).Log(new Elmah.Error(ex));
        ...
        /// <summary>
        /// Core of SaveBillingForClaimAI — persists the ServiceDetails/BillDetails/
        /// Deductions for a claim. Takes the acting identity (userRegionId/regionId/
        /// roleId) and the discount table as PARAMETERS rather than reading Session,
        /// so it can be invoked BOTH from the browser action (which passes Session
        /// values) AND from the session-less staging worker (which passes the
        /// configured system identity). No Session access happens in here.
        /// </summary>
        private bool SaveBillingForClaimAICore(
            long claimIdLong, int slNoInt,
            string hospitalBillAmount, string tariffAmount,
            string totalAmountApproved, string approvalBasis,
            int userRegionId, int regionId, int roleId,
            System.Data.DataTable dtDisc, out string vMessage)
        {
            ...
            try
            {
                ...
                // DeductionDetails - only if deduction > 0. Reason is chosen from the
                // approval basis ClaimAI reports (which figure the adjudicator picked
                // in Approvals):
                //   Total Bill Amount (manual)  -> ID 46  (Initial/Enhancement Approval Based on Received Estimate.)
                //   Tariff Amount (tariff)      -> ID 2418 (Restricted to agreed tariff (Do not collect from Member) |FT|)
                //   Ailment Sublimit (benefit)  -> ID 14  (Restricted to ailment limit as per policy |FT|)
                //   Hospital Estimate (bill) / Previously Paid (previous) / unknown -> ID 3
                //   (the ORIGINAL default). We deliberately do NOT use NULL here: an
                //   untested NULL DeductionReasonID risked being excluded from the
                //   Calculate section's deduction total downstream, which is exactly
                //   what broke after the first version of this patch. ID 3 is the
                //   proven-safe value the code always used before this feature existed.
                System.Data.DataTable dtDeductions = null;
                if (deductionAmt > 0)
                {
                    int deductionReasonId = 3; // safe default (Restricted to agreed tariff)
                    switch ((approvalBasis ?? "").Trim().ToLowerInvariant())
                    {
                        case "manual": deductionReasonId = 46; break; // Total Bill Amount
                        case "tariff": deductionReasonId = 2418; break; // Tariff Amount
                        case "benefit": deductionReasonId = 14; break; // Ailment Sublimit
                        case "bill":                                     // Hospital Estimate
                        case "previous":                                  // Previously Paid
                        default: deductionReasonId = 3; break; // unchanged from original
                    }
                    ...
                    dedRow["DeductionReasonID"] = deductionReasonId;
                    dedRow["IRDADeductionReasonID"] = DBNull.Value;
                    dedRow["FreeTextValue"] = "";
                    dtDeductions.Rows.Add(dedRow);
                }

                // Identity (userRegionId/regionId/roleId) and the discount table now
                // arrive as PARAMETERS — no Session access in the core.
                ...
                // PackageAmount (13th positional arg) was hard-coded "0", which is why
                // "Total Package Amt" stayed blank on every ClaimAI save while the rest
                // of the billing block populated. It now carries the Total Bill Amt -
                // the same hospAmt that fills BillAmount.
                ...
                return result > 0 || string.IsNullOrEmpty(vMessage);
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog.GetDefault(null).Log(new Elmah.Error(ex));
        ...
        /// <summary>
        /// PHASE 2a — session-less auto-Save for the staging worker.
        /// Runs the SAME core persist as the doctor's iframe Save, but supplies the
        /// acting identity from config (AutoApprovalSystemUserID / ...RegionID /
        /// ...RoleID) instead of a logged-in Session. Called by ProcessStagingClaims
        /// after a "done" result when the auto-approval decision is eligible.
        /// Returns true on success; never throws (logs + returns false).
        /// </summary>
        private bool AutoSaveBillingForStaging(
            long claimId, int slNo,
            string hospitalBillAmount, string tariffAmount,
            string totalAmountApproved, string approvalBasis)
        {
            try
            {
                // System identity from config. UserRegionID is REQUIRED (it becomes
                // ClosedBy / the acting user); RegionID and RoleID have safe fallbacks.
                string sysUserRegionStr = System.Configuration.ConfigurationManager.AppSettings["AutoApprovalSystemUserID"];
                if (string.IsNullOrWhiteSpace(sysUserRegionStr))
                {
                    TariffLog("[AutoApproval][Save] SKIPPED claim=" + claimId + " — AutoApprovalSystemUserID not configured.");
                    return false;
                }
                int sysUserRegionId;
                if (!int.TryParse(sysUserRegionStr.Trim(), out sysUserRegionId) || sysUserRegionId <= 0)
                {
                    TariffLog("[AutoApproval][Save] SKIPPED claim=" + claimId + " — AutoApprovalSystemUserID is not a valid id.");
                    return false;
                }
                int sysRegionId;
                if (!int.TryParse((System.Configuration.ConfigurationManager.AppSettings["AutoApprovalSystemRegionID"] ?? "").Trim(), out sysRegionId))
                    sysRegionId = sysUserRegionId; // fallback
                int sysRoleId;
                if (!int.TryParse((System.Configuration.ConfigurationManager.AppSettings["AutoApprovalSystemRoleID"] ?? "").Trim(), out sysRoleId))
                    sysRoleId = 20; // same default the browser path uses

                // No Session["ClaimDiscount"] in the worker — pass an empty discount
                // table (the doctor path also runs with an empty/absent one for AI saves).
                System.Data.DataTable dtDisc = null;

                string msg;
                bool ok = SaveBillingForClaimAICore(
                    claimId, slNo,
                    hospitalBillAmount, tariffAmount, totalAmountApproved, approvalBasis,
                    sysUserRegionId, sysRegionId, sysRoleId, dtDisc, out msg);
                TariffLog("[AutoApproval][Save] claim=" + claimId + " ok=" + ok
                    + " approved=" + (totalAmountApproved ?? "") + " basis=" + (approvalBasis ?? "") + " msg=" + (msg ?? ""));
                return ok;
            }
            catch (Exception ex)
            {
                TariffLog("[AutoApproval][Save] FAILED claim=" + claimId + ": " + ex.Message);
                return false;
            }
        }

        /// <summary>
        /// Resolve the exact Claimsdetails.ID (row PK) for a specific (ClaimID, SlNo).
        /// The clinical save finds its row by "TOP 1 ... ORDER BY SlNo DESC" when given a
        /// ClaimID, which can target a DIFFERENT SlNo than billing/coding (and than the
        /// UI shows) on multi-SlNo claims. Passing the exact row ID forces the correct
        /// row. Returns 0 when not found.
        /// </summary>
        private long ResolveClaimDetailsRowId(string connStr, long claimId, int slNo)
        {
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    // Exact-SlNo match, falling back to the lowest-Slno row, in ONE
                    // ordered SELECT instead of two sequential queries.
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.CommandText = "USP_ClaimAI_ResolveClaimRow";
                    cmd.Parameters.AddWithValue("@ClaimID", claimId);
                    cmd.Parameters.AddWithValue("@SlNo", slNo);
                    using (var rdr = cmd.ExecuteReader())
                    {
                        if (rdr.Read() && rdr["RowId"] != DBNull.Value)
                            return Convert.ToInt64(rdr["RowId"]);
                    }
                }
            }
            catch (Exception ex) { TariffLog("[AutoApproval][Save] row id resolve failed claim=" + claimId + ": " + ex.Message); }
            return 0;
        }

        /// <summary>
        /// Look up the numeric TPAProcedure ID for a resolved Level3 procedure NAME
        /// (ClaimAI resolves the name using its tpaProcedures rules; only Spectra can
        /// map name -> id via the TPAProcedures master). Returns 0 when not found.
        /// </summary>
        private int ResolveTpaProcedureIdByName(string connStr, string level3Name)
        {
            if (string.IsNullOrWhiteSpace(level3Name)) return 0;
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    cmd.CommandText = "SELECT TOP 1 ID FROM TPAProcedures WHERE LTRIM(RTRIM(Level3)) = @name AND ISNULL(Deleted,0) = 0 ORDER BY ID";
                    cmd.Parameters.AddWithValue("@name", level3Name.Trim());
                    var v = cmd.ExecuteScalar();
                    if (v != null && v != DBNull.Value) return Convert.ToInt32(v);
                }
            }
            catch (Exception ex) { TariffLog("[AutoApproval][Save] procedure id lookup failed for \"" + level3Name + "\": " + ex.Message); }
            return 0;
        }

        /// <summary>
        /// Resolve the approved facility for a staged auto-approval. ClaimAI's default
        /// rule is "approved accommodation = availed accommodation", so we use the
        /// claim's requested/availed facility (Claimsdetails.ReqFacilityID). Returns
        /// null when unavailable (the save then keeps the existing value).
        /// </summary>
        private string ResolveApprovedFacilityForStaging(string connStr, long claimId)
        {
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    cmd.CommandText = "SELECT TOP 1 ISNULL(ApprovedFacilityID, ReqFacilityID) AS Fac FROM Claimsdetails WHERE ClaimID=@ID AND ISNULL(Deleted,0)=0 ORDER BY SlNo DESC";
            ...
            catch (Exception ex) { TariffLog("[AutoApproval][Save] facility resolve failed claim=" + claimId + ": " + ex.Message); }
            return null;
        }

        /// <summary>
        /// Record the auto-save outcome on ClaimAI_Results so failures are queryable.
        /// The stage is already 5 by the time this runs; this does NOT change the stage,
        /// it just annotates what happened (success / failed / skipped + any error).
        /// </summary>
        private void RecordAutoSaveResult(string connStr, long claimId, int slNo, string status, string error)
        {
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    cmd.CommandText = @"UPDATE dbo.ClaimAI_Results
            ...
            {
                TariffLog("[AutoApproval][Save] RecordAutoSaveResult failed claim=" + claimId + ": " + ex.Message);
            }
        }

        /// <summary>
        /// PHASE 2a (full) — replays ALL FOUR saves the doctor's iframe "Save" performs,
        /// session-less, for a staged eligible claim:
        ///   1. Billing   (SaveBillingForClaimAICore, config identity)
        ///   2. Clinical  (SaveClinicalDetailsForClaimAI, via _stagingInternalCall)
        ///   3. Coding    (SaveCodingRowForClaimAI, via _stagingInternalCall)
        ///   4. Hospital. (SaveHospitalizationDetailsForClaimAI, via _stagingInternalCall)
        /// Values are derived from the ClaimAI analysis JSON. Fields that the browser
        /// resolves via live lookups (approved facility, some dropdowns) may be absent
        /// server-side — those saves run with whatever is derivable and are logged so
        /// gaps are visible. Returns true if the BILLING save (the money step) succeeded.
        /// </summary>
        private bool AutoPerformFullSaveForStaging(
            long claimId, int slNo, string disease, string analysisJson,
            string hospEstimate, string tariffAmount, string approvedAmount, string approvalBasis,
            out string saveStatus, out string saveError)
        {
            bool billingOk = false;
            bool prevFlag = _stagingInternalCall;
            var saveErrors = new System.Collections.Generic.List<string>();
            saveStatus = "failed";
            saveError = null;
            try
            {
                // PIN-TO-PIN: ClaimAI built the EXACT save payloads (same derivation the
                // doctor's iframe uses) and put them in analysis.autoSavePayloads. We replay
                // those values verbatim — no server-side re-derivation of amounts/diagnosis.
                dynamic an = null;
                try { if (!string.IsNullOrWhiteSpace(analysisJson)) an = Newtonsoft.Json.JsonConvert.DeserializeObject(analysisJson); }
                catch { an = null; }
                dynamic pl = null;
                try { pl = an?.autoSavePayloads; } catch { pl = null; }

                string connStrLocal = GetStagingConnString();

                // ── 1) BILLING (setBillingDetails) — via the config-identity core ──────
                // Use the payload values verbatim; fall back to the passed args only if the
                // payload is somehow absent.
                string billHosp = hospEstimate, billTariff = tariffAmount,
                       billApproved = approvedAmount, billBasis = approvalBasis;
                try
                {
                    if (pl?.billing != null)
                    {
                        billHosp = (string)(pl.billing.hospitalBillAmount?.ToString() ?? billHosp);
                        billTariff = (string)(pl.billing.tariffAmount?.ToString() ?? billTariff);
                        billApproved = (string)(pl.billing.totalAmountApproved?.ToString() ?? billApproved);
                        billBasis = (string)(pl.billing.approvalBasis?.ToString() ?? billBasis);
                    }
                }
                catch { }
                billingOk = AutoSaveBillingForStaging(claimId, slNo, billHosp, billTariff, billApproved, billBasis);
                if (!billingOk) saveErrors.Add("billing save failed");

                // The next saves reuse the existing action methods with the session gate
                // bypassed via _stagingInternalCall.
                _stagingInternalCall = true;

                // Approved facility: prefer the claim's existing/availed facility; if the
                // claim has none, fall back to the disease default facility ClaimAI put in
                // the payload (cataract Day-care = 195) — mirrors the manual save's `|| 195`.
                string resolvedFacilityId = ResolveApprovedFacilityForStaging(connStrLocal, claimId);
                if (string.IsNullOrWhiteSpace(resolvedFacilityId))
                {
                    try
                    {
                        string _defFac = (string)(pl?.approvedAccommodation?.defaultFacilityId?.ToString());
                        if (!string.IsNullOrWhiteSpace(_defFac) && _defFac != "0") resolvedFacilityId = _defFac;
                    }
                    catch { }
                }
                TariffLog("[AutoApproval][Save] facility resolved=" + (resolvedFacilityId ?? "<null>") + " claim=" + claimId);

                // ── 2) CLINICAL (setClinicalDetails) — replay exact payload values ────
                try
                {
                    string diagnosis = null, lineOfTreatment = null, presentComplaint = null,
                           processingRemarks = null, doctorNotes = null;
                    try { if (pl?.clinical != null) TariffLog("[AutoApproval][Save] RAW pl.clinical=" + Newtonsoft.Json.JsonConvert.SerializeObject(pl.clinical)); else TariffLog("[AutoApproval][Save] pl.clinical is NULL claim=" + claimId); } catch (Exception _le) { TariffLog("[AutoApproval][Save] pl.clinical log failed: " + _le.Message); }
                    if (pl?.clinical != null)
                    {
                        try { diagnosis = (string)(pl.clinical.diagnosis?.ToString()); } catch { }
                        try { lineOfTreatment = (string)(pl.clinical.lineOfTreatment?.ToString()); } catch { }
                        try { presentComplaint = (string)(pl.clinical.presentingComplaint?.ToString()); } catch { }
                        try { processingRemarks = (string)(pl.clinical.processingRemarks?.ToString()); } catch { }
                        try { doctorNotes = (string)(pl.clinical.doctorNotes?.ToString()); } catch { }
                    }
                    // DIAGNOSTIC: log exactly what the worker read from pl.clinical, so a
                    // missing diagnosis/LoT can be traced from the Spectra tariff log alone
                    // (App_Data/Logs/TariffSelection_*.log) without the ClaimAI console.
                    TariffLog("[AutoApproval][Save] clinical INPUTS claim=" + claimId
                        + " | diagnosis=[" + (diagnosis ?? "<null>") + "]"
                        + " | lineOfTreatment=[" + (lineOfTreatment ?? "<null>") + "]"
                        + " | presentComplaint=[" + (presentComplaint ?? "<null>") + "]"
                        + " | processingRemarks=[" + (processingRemarks ?? "<null>") + "]"
                        + " | facility=[" + (resolvedFacilityId ?? "<null>") + "]");
                    // Target the EXACT Claimsdetails row for (claimId, slNo) so clinical
                    // details land on the same SlNo row billing/coding use (and the UI shows),
                    // not "TOP 1 by SlNo DESC".
                    long clinicalRowId = ResolveClaimDetailsRowId(connStrLocal, claimId, slNo);
                    string clinicalIdParam = clinicalRowId > 0 ? clinicalRowId.ToString() : claimId.ToString();
                    TariffLog("[AutoApproval][Save] clinical rowId=" + clinicalRowId + " (claim=" + claimId + " slNo=" + slNo + ")");
                    SaveClinicalDetailsForClaimAI(
                        clinicalIdParam, diagnosis, lineOfTreatment,
                        presentComplaint, processingRemarks, doctorNotes,
                        /*hospTreatmentTypeId*/ null, resolvedFacilityId, /*patientConditionId*/ "269");
                    TariffLog("[AutoApproval][Save] clinical saved claim=" + claimId);
                }
                catch (Exception ce) { saveErrors.Add("clinical: " + ce.Message); TariffLog("[AutoApproval][Save] clinical FAILED claim=" + claimId + ": " + ce.Message); }

                // ── 3) CODING (setIcdSlots) — replay exact payload; resolve proc name->id ─
                try
                {
                    string resolvedProcName = null, lastIcdCode = null, eligibleAmt = null, packageAmt = null;
                    if (pl?.icd != null)
                    {
                        try { resolvedProcName = (string)(pl.icd.resolvedTpaProcedureName?.ToString()); } catch { }
                        try { lastIcdCode = (string)(pl.icd.lastIcdCode?.ToString()); } catch { }
                        try { eligibleAmt = (string)(pl.icd.eligibleAmount?.ToString()); } catch { }
                        try { packageAmt = (string)(pl.icd.packageAmount?.ToString()); } catch { }
                    }
                    int resolvedProcId = ResolveTpaProcedureIdByName(connStrLocal, resolvedProcName);
                    TariffLog("[AutoApproval][Save] procName=\"" + (resolvedProcName ?? "") + "\" -> id=" + resolvedProcId + ", facility=" + (resolvedFacilityId ?? "null"));
                    SaveCodingRowForClaimAI(
                        claimId.ToString(), slNo.ToString(),
                        resolvedProcId > 0 ? resolvedProcId.ToString() : null,
                        lastIcdCode, eligibleAmt, packageAmt, disease);
                    TariffLog("[AutoApproval][Save] coding saved claim=" + claimId);
                }
                catch (Exception ce) { saveErrors.Add("coding: " + ce.Message); TariffLog("[AutoApproval][Save] coding FAILED claim=" + claimId + ": " + ce.Message); }

                // ── 4) HOSPITALIZATION (setApprovedAccommodation) — facility + billNo ─
                try
                {
                    ...
                    SaveHospitalizationDetailsForClaimAI(
                                    claimId.ToString(), slNo.ToString(), resolvedFacilityId,
                                    /*availedFacilityId*/ null,   // staged path has no dropdown to read
                                    /*billNo*/ "AI",
                                    stgNature, stgAdmit);
                    TariffLog("[AutoApproval][Save] hospitalization saved claim=" + claimId
                        + " nature=" + stgNature + " admissionType=" + (stgAdmit == "" ? "(unset)" : stgAdmit));
                }
                catch (Exception ce) { saveErrors.Add("hospitalization: " + ce.Message); TariffLog("[AutoApproval][Save] hospitalization FAILED claim=" + claimId + ": " + ce.Message); }
            }
            catch (Exception ex)
            {
                saveErrors.Add("fatal: " + ex.Message);
                TariffLog("[AutoApproval][Save] full-save FAILED claim=" + claimId + ": " + ex.Message);
            }
            finally
            {
                _stagingInternalCall = prevFlag;
            }
            // Status: success only when billing (the money step) succeeded AND no other
            // save errored. Otherwise failed, with the collected reasons.
            if (billingOk && saveErrors.Count == 0)
            {
                saveStatus = "success";
                saveError = null;
            }
            ...
            {
                saveStatus = "failed";
                saveError = string.Join("; ", saveErrors);
            }
            return billingOk;
        }
            ...
            string patientConditionId = null)
        {
            try
            {
                if (!_stagingInternalCall && Session[SessionValue.UserRegionID] == null)
                    return Json(new { success = false, message = "Session expired" });
                if (!_stagingInternalCall && Convert.ToString(Session["ClaimStageID"]) != "5")
                    return Json(new { success = false, message = "AI Summary is read-only outside the Adjudication stage." });
                ...
                try
                {
                    // One round trip: row-id resolution (including the ClaimID fallback)
                    // and the update all happen inside USP_ClaimAI_SaveClinicalDetails.
                    // NULL means "leave this column unchanged" - the same fields the old
                    // dynamic SET list skipped when they were blank.
                    Func<string, object> Txt = v =>
                        string.IsNullOrWhiteSpace(v) ? (object)DBNull.Value : v.Trim();

                    object treatmentTypeParam = DBNull.Value;
                    int typeId;
                    if (!string.IsNullOrWhiteSpace(hospTreatmentTypeId)
                        && int.TryParse(hospTreatmentTypeId.Trim(), out typeId) && typeId > 0)
                        treatmentTypeParam = typeId;

                    // Accepts int or long exactly as the old code did, and "0" still means
                    // "not supplied" rather than a facility id.
                    object approvedFacilityParam = DBNull.Value;
                    long facilityIdLong;
                    if (!string.IsNullOrWhiteSpace(approvedFacilityId)
                        && approvedFacilityId.Trim() != "0"
                        && long.TryParse(approvedFacilityId.Trim(), out facilityIdLong)
                        && facilityIdLong > 0)
                        approvedFacilityParam = facilityIdLong;

                    // Always written, defaulting to 269 (Recovered), as before.
                    int patientConditionIdInt = 269;
                    ...
                    using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                    {
                        ...
                        {
                            cmd.CommandType = System.Data.CommandType.StoredProcedure;
                            cmd.CommandText = "USP_ClaimAI_SaveClinicalDetails";
                            cmd.CommandTimeout = 60;
                            cmd.Parameters.AddWithValue("@ClaimDetailsID", numericId);
                            cmd.Parameters.AddWithValue("@PatientConditionID", patientConditionIdInt);
                            cmd.Parameters.AddWithValue("@Diagnosis", Txt(probableDiagnosis));
                            cmd.Parameters.AddWithValue("@PlanOfTreatment", Txt(probableLineOfTreatment));
                            cmd.Parameters.AddWithValue("@PresentComplaint", Txt(presentComplaint));
                            cmd.Parameters.AddWithValue("@ExecutiveNotes", Txt(processingRemarks));
                            cmd.Parameters.AddWithValue("@DoctorNotes", Txt(doctorNotes));
                            cmd.Parameters.Add("@TreatmentTypeID", System.Data.SqlDbType.Int).Value = treatmentTypeParam;
                            cmd.Parameters.Add("@ApprovedFacilityID", System.Data.SqlDbType.BigInt).Value = approvedFacilityParam;

                            using (var rdr = cmd.ExecuteReader())
                            {
                                if (rdr.Read())
                                {
                                    rowsAffected = Convert.ToInt32(rdr["RowsAffected"]);
                                    rowFound = Convert.ToBoolean(rdr["RowFound"]);
                                }
                            }
                        }
                    }
            ...
            string admissionTypeId = null)
        {
            try
            {
                ...
                // One round trip on one connection, replacing three UPDATEs and two
                // debug read-back SELECTs across two connections.
                // Parameter nulls carry the old guards: a null facility id skips that
                // UPDATE entirely; a null nature/admission id leaves the column alone.
                int rowsAffected = 0;
                ...
                // AdmissionTypeID is tinyint - anything outside 1-255 would throw and lose
                // the whole save, so skip it and log instead.
                int admitIdVal;
                ...
                int facIdVal;
                object approvedParam =
                    !string.IsNullOrWhiteSpace(approvedFacilityId)
                    && int.TryParse(approvedFacilityId.Trim(), out facIdVal)
                    && facIdVal > 0
                        ? (object)facIdVal
                        : DBNull.Value;
                ...
                int availFacIdVal;
                object availedParam =
                    !string.IsNullOrWhiteSpace(availedFacilityId)
                    && int.TryParse(availedFacilityId.Trim(), out availFacIdVal)
                    && availFacIdVal > 0
                        ? (object)availFacIdVal
                        : DBNull.Value;
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    {
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
                        cmd.CommandText = "USP_ClaimAI_SaveHospitalization";
                        cmd.CommandTimeout = 60;
                        cmd.Parameters.AddWithValue("@ClaimID", claimIdLong);
                        cmd.Parameters.AddWithValue("@SlNo", slNoInt);
                        cmd.Parameters.AddWithValue("@BillNo", billNoVal);
                        cmd.Parameters.Add("@NatureOfTreatmentTypeID", System.Data.SqlDbType.Int).Value = natureParam;
                        cmd.Parameters.Add("@AdmissionTypeID", System.Data.SqlDbType.TinyInt).Value = admitParam;
                        cmd.Parameters.Add("@ApprovedFacilityID", System.Data.SqlDbType.Int).Value = approvedParam;
                        cmd.Parameters.Add("@AvailedFacilityID", System.Data.SqlDbType.Int).Value = availedParam;
                        ...
                        using (var rdr = cmd.ExecuteReader())
                        {
                            ...
                            {
                                rowsAffected = Convert.ToInt32(rdr["RowsAffected"]);
                                debugFacilityChanged = rdr["DebugIsFacilityChanged"] != DBNull.Value
                                    ? rdr["DebugIsFacilityChanged"].ToString() : "null";
                                debugApproved = "ApprovedFacilityID="
                                    + (rdr["DebugApprovedFacilityID"] != DBNull.Value
                                        ? rdr["DebugApprovedFacilityID"].ToString() : "null");
                            }
                        }
                    }
                }
                TariffLog("[ClaimAI] Hospitalization saved claim=" + claimIdLong
                    + " rows=" + rowsAffected
                    + " nature=" + (natureParam == DBNull.Value ? "unchanged" : natureParam.ToString())
                    + " admissionType=" + (admitParam == DBNull.Value ? "unchanged" : admitParam.ToString()));
            ...
            string claimType = null)   // Phase 6: disease type passed from JS; null = fall back to cataract default
        {
            try
            {
                ...
                // The truncation walk (H25.813 -> H25.81 -> H25.8 -> H25 -> H2 -> H) is
                // pure string work and needs no database, so the whole candidate list is
                // built here and ONE query picks the winner - previously this was one
                // round trip per step, up to six per save.
                int icdNumericId = 0;
                if (!string.IsNullOrWhiteSpace(icdCodeStr))
                {
                    // Identical shortening rules to the old loop, in the same order.
                    var icdCandidates = new System.Collections.Generic.List<string>();
                    string code = icdCodeStr.Trim();
                    while (code.Length > 0 && icdCandidates.Count < 20)
                    {
                        icdCandidates.Add(code);
                        if (code.Contains("."))
                        {
                    ...
                    if (icdCandidates.Count > 0)
                    {
                        using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                        {
                            conn.Open();
                            var icdCmd = conn.CreateCommand();
                            icdCmd.CommandType = System.Data.CommandType.StoredProcedure;
                            icdCmd.CommandText = "USP_ClaimAI_ResolveICD10";
                            icdCmd.CommandTimeout = 30;
                            icdCmd.Parameters.AddWithValue("@CodeCandidates", string.Join(",", icdCandidates.ToArray()));
                            var sc = icdCmd.ExecuteScalar();
                            if (sc != null && sc != DBNull.Value) icdNumericId = Convert.ToInt32(sc);
                        }
                    }
                    System.Diagnostics.Debug.WriteLine(
                        "[ClaimAI] ICD10 resolve: '" + icdCodeStr.Trim() + "' -> " + icdNumericId
                        + " (tried " + icdCandidates.Count + " candidate(s) in one round trip)");
                }
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    catch (System.Data.SqlClient.SqlException sqlEx) when (sqlEx.Message.IndexOf("BillingType_P51", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        ...
                        // Get UserRegionID from session; in staging (session-less) use the
                        // configured system identity so the coding row is owned correctly.
                        int userRegionId = 1;
                        if (_stagingInternalCall)
                        {
                            int _sysUid;
                            if (int.TryParse((System.Configuration.ConfigurationManager.AppSettings["AutoApprovalSystemUserID"] ?? "").Trim(), out _sysUid) && _sysUid > 0)
                                userRegionId = _sysUid;
                        }
                        else if (Session[SessionValue.UserRegionID] != null)
                        {
                        ...
                        if (tpaProcId > 0)
                        {
                            try
                            {
                                ...
                                // One round trip instead of up to nine. The ancestor walk,
                                // the walk-down to a leaf child, the L1/L2/L3 lookup and the
                                // PCS parent-code fallback are all inside
                                // USP_ClaimAI_ResolveTPAProcedure now.
                                var tpaCmd = conn.CreateCommand();
                                tpaCmd.CommandType = System.Data.CommandType.StoredProcedure;
                                tpaCmd.CommandText = "USP_ClaimAI_ResolveTPAProcedure";
                                tpaCmd.CommandTimeout = 30;
                                tpaCmd.Parameters.AddWithValue("@TPAProcedureID", tpaProcId);
                                int ancestorCount = 0;
                                using (var rdr = tpaCmd.ExecuteReader())
                                {
                                    if (rdr.Read())
                                    {
                                        ...
                                        level2Id = Convert.ToInt32(rdr["L2ID"]);
                                        level1Id = Convert.ToInt32(rdr["L1ID"]);
                                        ancestorCount = Convert.ToInt32(rdr["AncestorCount"]);
                                        actualL3Id = Convert.ToInt32(rdr["ResolvedFromID"]);
                                    }
                                }
                                System.Diagnostics.Debug.WriteLine(
                                    "[ClaimAI] TPA resolve: input=" + tpaProcId +
                                    " ancestors=" + ancestorCount +
                                    " resolvedL3=" + actualL3Id);
        ...
        public ActionResult GetClaimFieldsForValidation(string claimId)
        {
            try
            {
                ...
                // One round trip on one connection, replacing four queries across two
                // connections. All four asked about the same claim.
                // NOTE: documentDate / dischargeDate still come back null - the old
                // query read dateofbill / dateofdischarge from Claimsdetails, where
                // those columns do not exist, so it threw on every call and the
                // surrounding catch swallowed it. Behaviour preserved deliberately;
                // see FIX_SP_Phase2_ValidationFields.md before changing it.
                string age = null;
                string hospitalName = null;
                string documentDate = null;
                string dischargeDate = null;
                string diagnosis = null;
                string presentComplaint = null;
                string processingRemarks = null;
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    {
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
                        cmd.CommandText = "USP_ClaimAI_GetValidationFields";
                        cmd.CommandTimeout = 30;
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                Func<string, string> S = col =>
                                    reader[col] != DBNull.Value ? reader[col].ToString().Trim() : null;

                                age = S("Age");
                                hospitalName = S("ProviderName");
                                documentDate = S("DocumentDate");
                                dischargeDate = S("DischargeDate");
                                diagnosis = S("Diagnosis");
                                presentComplaint = S("PresentComplaint");
                                processingRemarks = S("ExecutiveNotes");
                                doctorNotes = S("DoctorNotes");
                                approvedFacilityId = S("ApprovedFacilityID");
                                patientConditionId = S("MillimanConditionID");
                                reqFacilityId = S("ReqFacilityID");
                                reqFacilityName = S("ReqFacilityName");
                                approvedFacilityName = S("ApprovedFacilityName");
                            }
                        }
                    }
                }
        ...
        // REMOVED: IsClaimAISummaryAllowed. It was a second gate on the same question
        // stage-52 promotion already answers, and both callers in Index.cshtml are gone.
        ...
        public ActionResult FinalizeClaimAISave(string claimId = null, string slNo = null)
        {
            try
            {
                ...
                // IsAprvFacilitychanged is still NOT touched here - overriding that one is
                // what made Claim Actions disappear after manual edits saved through Spectra.
                //
                // BillingCorrection IS advanced to 2, because the in-memory override
                // (basicData[0].BillingCorrection = 2) only survives while the page is open.
                // Reopen the claim - in Refer to CRM, say - and basicData is re-read from
                // the DB, where the value is still 1, and Claim Actions warns
                // "please modify Coding details".
                //
                // 1 = billing changed, coding stale (Usp_ClaimBillDetails_Insert)
                // 2 = coding re-saved, in sync   (USP_CLA_SaveClaimCodingData)
                //
                // The ClaimAI save DOES write both billing and coding, so 2 is the correct
                // end state. It does not get there reliably on its own for two reasons: the
                // three saves are independent async calls whose completion order is not
                // guaranteed (billing finishing last overwrites coding's 2 with 1), and the
                // coding fallback INSERT at ~line 12075 bypasses USP_CLA_SaveClaimCodingData
                // entirely. Setting it here, after all saves have completed, is immune to both.
                //
                // Guarded to 1 -> 2 only: any other value was set by a native Spectra save
                // and is left alone.
                try
                {
                    using (var bcConn = new System.Data.SqlClient.SqlConnection(connStr))
                    {
                        bcConn.Open();
                        var bcCmd = bcConn.CreateCommand();
                        bcCmd.CommandTimeout = 60;
                        // Deliberately NOT filtered by Slno.
                        //
                        // The post-sequence call runs after a page reload and reads the Slno
                        // from $('#hdnClaimSlNo'), which is not reliably the row the billing
                        // save touched. A mismatch made this UPDATE match ZERO rows - no
                        // error, no warning, just a flag left at 1 and the "change in Billing
                        // details ... modify Coding details" warning appearing on a claim that
                        // had saved correctly.
                        //
                        // The ISNULL(BillingCorrection,0) = 1 guard is KEPT, and is what makes
                        // this safe to widen: only rows already in the "billing changed, coding
                        // stale" state are flipped. Rows sitting at 0 or 2 are untouched.
                        //
                        // Trade-off worth knowing: on a claim with several Slno rows, a row
                        // whose coding genuinely IS stale would also be marked in-sync and its
                        // warning suppressed. Accepted because ClaimAI saves the claim as a
                        // whole - but if per-extension billing is ever adjudicated separately,
                        ...
                        bcCmd.CommandText = @"
                            ...
                            WHERE  ClaimID = @ClaimID
                              AND  ISNULL(Deleted, 0) = 0
                              AND  ISNULL(BillingCorrection, 0) = 1";
                        bcCmd.Parameters.AddWithValue("@ClaimID", claimIdLong);
                        int bcRows = bcCmd.ExecuteNonQuery();
                        TariffLog("[ClaimAI] FinalizeClaimAISave: BillingCorrection -> 2 for claim "
                            + claimIdLong + " across ALL Slno (rows affected: " + bcRows + ")");
                    }
                }
                catch (Exception bcEx)
                {
                    // Never fail the save for this - the amounts are already committed.
                    TariffLog("[ClaimAI] FinalizeClaimAISave: BillingCorrection update failed for claim "
                        + claimIdLong + ": " + bcEx.Message);
                }
        ...
        // ── ClaimAI frame endpoints (tokenized, mirrors Common/ProviderTariff) ──
        // ClaimAI's /api/member-view and /api/provider-details build a
        // {SPECTRA_BASE_URL}/MedicalScrutiny/{X}Frame?key={encrypted} URL. The key
        // is Encrypt("MV|memberPolicyID|claimId|siType") / Encrypt("PD|claimId|
        // providerID|memberPolicyID") with AppSettings["URLEncryptionKey"] — the
        // exact same scheme ProviderTariff uses. We decrypt it here, require a
        // valid session (same as ProviderTariff), and render the frame views.

        [HttpGet]
        public ActionResult MemberViewFrame(string key)
        {
            if (Session[SessionValue.UserRegionID] == null)
                return RedirectToAction("MCareLogin", "Account");
            try
            {
                string clear = new MasterUtilsBL().Decrypt(
                    (key ?? "").Replace(" ", "+"),
                    Convert.ToString(ConfigurationManager.AppSettings["URLEncryptionKey"]));
                string[] p = clear.Split('|');
                // p[0] = "MV" tag
                long memberPolicyID = long.Parse(p[1]);
                long claimId = (p.Length > 2) ? long.Parse(p[2]) : 0;
                int siTypeID = (p.Length > 3) ? int.Parse(p[3]) : 5;

                ViewBag.MemberPolicyID = memberPolicyID;
                ViewBag.ClaimID = claimId;
                ViewBag.SITypeID = siTypeID;
                return View("MemberViewFrame");
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog.GetDefault(null).Log(new Elmah.Error(ex));
                return new HttpStatusCodeResult(400, "Invalid key");
            }
        }

        [HttpGet]
        public ActionResult ProviderDetailsFrame(string key)
        {
            if (Session[SessionValue.UserRegionID] == null)
                return RedirectToAction("MCareLogin", "Account");
            try
            {
                ...
                string[] p = clear.Split('|');
                // p[0] = "PD" tag
                long claimId = long.Parse(p[1]);
                long providerID = long.Parse(p[2]);
                long memberPolicyID = (p.Length > 3) ? long.Parse(p[3]) : 0;

                ViewBag.ClaimID = claimId;
                ViewBag.ProviderID = providerID;
                ViewBag.MemberPolicyID = memberPolicyID;
                return View("ProviderDetailsFrame");
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog.GetDefault(null).Log(new Elmah.Error(ex));
        ...
        public ActionResult GetBSIForClaimAI(string claimId)
        {
            ...
            try
            {
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        // The old inline query compared CAST(c.ID AS VARCHAR(50)) to a
                        // string, which defeated the index on Claims.ID and scanned the
                        // table on every BSI request. The procedure takes a bigint.
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
                        cmd.CommandText = "USP_ClaimAI_GetBSIContext";
                        cmd.CommandTimeout = 60;
                        long bsiClaimIdLong;
                        if (!long.TryParse((claimId ?? "").Trim(), out bsiClaimIdLong) || bsiClaimIdLong <= 0)
                        {
                            res.Success = false;
                            res.Message = "Claim not found: " + claimId;
                            return Json(res, JsonRequestBehavior.AllowGet);
                        }
                        cmd.Parameters.AddWithValue("@ClaimID", bsiClaimIdLong);
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                slNo = reader["ClaimsSlNo"] != DBNull.Value
                                    ? Convert.ToByte(reader["ClaimsSlNo"]) : (byte)1;
                            }
                        }
                    }
                }
        ...
        public string ClaimAudit_Validate(string ClaimDetails, bool isApprove, string PolicyType, string MainMemberPolicyID, string PolicyID, string ProviderID,
                  string BrokerID, string PayerID, string CorporateID, string InsuranceCompanyID, decimal excesssuminsured, decimal SanctionedAmount, string QMSID, string QMSAdminID, bool? skipAudit, int? createnewtop = 0, string Isrefertocrm = "0")
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    ...
                    objActionIteams.ReasonIDs_P = "220";
                    dsResult = _objMadicalScrutinyVM.ClaimAudit_Validate(objActionIteams, isApprove, out msg, Convert.ToInt16(PolicyType), PayeeType, Convert.ToString(JObject["NomineePayeeName"]), null, "2");
                    return Newtonsoft.Json.JsonConvert.SerializeObject(msg);
                }
                else
                {
        ...
        [Authorize]
        public string SkipMedicalScrutinyDropdownDetails()
        {
            try
            {
                if (Session[SessionValue.UserRegionID] != null)
                {
                    return Newtonsoft.Json.JsonConvert.SerializeObject(_objMadicalScrutinyVM.SkipMedicalScrutinyDropdownDetails());
                }
                else
                {
                    return "ErrorCode#1";
        ...
        [HttpPost]
        public JsonResult GetOPDServiceBillingDetails(string MemberPolicyID, string SITypeID, string MemberSIID, string SICategoryID, string ClaimID, string ClaimSlNo)
        {
            try
            {
                if (Session[SessionValue.RegionID] != null)
                {
                    ...
                    DataSet dt = new DataSet();
                    dt = _objMadicalScrutinyVM.GetOPDServiceBillingDetails(MemberPolicyID, SITypeID, MemberSIID, SICategoryID, ClaimID, ClaimSlNo);
                    return this.Json(JsonConvert.SerializeObject(dt, Newtonsoft.Json.Formatting.Indented), JsonRequestBehavior.AllowGet);
        ...
        [HttpPost]
        public JsonResult SaveOPDBillingDetails(
            string ClaimID,
            string Slno,
            string IswithInSI,
            string ClaimOPDBillingDetailsType,
            string ClaimOPDBillingDetailsTxnType,
            string ClaimOPDDeductionsDetailsType)
        {
            try
            {
                if (Session[SessionValue.RegionID] == null)
                    return this.Json(
                        JsonConvert.SerializeObject("ErrorCode#1", Newtonsoft.Json.Formatting.Indented),
                        JsonRequestBehavior.AllowGet);

                int createdBy = Convert.ToInt32(Session[SessionValue.UserRegionID]);

                // Deserialize using ViewModel nested classes — resolves CS1503
                var billingDetails =
                    JsonConvert.DeserializeObject<List<MedicalScrutinyViewModel.ClaimOPDBillingDetailsJson>>(
                        ClaimOPDBillingDetailsType ?? "[]");

                var billingTxnDetails =
                    JsonConvert.DeserializeObject<List<MedicalScrutinyViewModel.ClaimOPDBillingDetailsTxnJson>>(
                        ClaimOPDBillingDetailsTxnType ?? "[]");

                var deductionDetails =
                    JsonConvert.DeserializeObject<List<MedicalScrutinyViewModel.ClaimOPDDeductionsDetailsJson>>(
                        ClaimOPDDeductionsDetailsType ?? "[]");

                bool result = _objMadicalScrutinyVM.SaveOPDBillingDetails(
                    Convert.ToInt64(ClaimID),
                    Convert.ToInt32(Slno),
                    Convert.ToInt32(IswithInSI),
                    createdBy,
                    billingDetails,
                    billingTxnDetails,
                    deductionDetails);
                ...
                return this.Json(
                    JsonConvert.SerializeObject(
                        new { Success = result, Message = result ? "Saved successfully." : "Save failed." },
        ...
        [HttpPost]
        public JsonResult ReleaseMemberOPDBalances(string ClaimID, string Slno, string flag)
        {
            try
            {
                if (Session[SessionValue.RegionID] == null)
                ...
                bool result = _objMadicalScrutinyVM.ReleaseMemberOPDBalances(Convert.ToInt64(ClaimID), Convert.ToInt32(Slno), Convert.ToInt32(flag));

                return this.Json(
                    JsonConvert.SerializeObject(
                        new { Success = result, Message = result ? "Saved successfully." : "Save failed." }, Newtonsoft.Json.Formatting.Indented), JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Elmah.ErrorLog errorLog = Elmah.ErrorLog.GetDefault(null);
        ...
        [HttpPost]
        public JsonResult GetOPDCodingCalculation(string ClaimID, string Slno, string servicetypeid, string TPAProcID, string TPAProcedureID, string ClaimSubmission_Services)
        {
            try
            {
                if (Session[SessionValue.RegionID] != null)
                {
                    var billingDetails =
                    JsonConvert.DeserializeObject<List<MedicalScrutinyViewModel.ClaimSubmission_Services>>(
                        ClaimSubmission_Services ?? "[]");
                    DataSet dt = new DataSet();
                    dt = _objMadicalScrutinyVM.GetOPDCodingCalculation(ClaimID, Slno, servicetypeid, TPAProcID, TPAProcedureID, billingDetails);
                    return this.Json(JsonConvert.SerializeObject(dt, Newtonsoft.Json.Formatting.Indented), JsonRequestBehavior.AllowGet);
                }
                else
                {
        ...
        [HttpPost]
        public JsonResult OPDCodingCalculationOtherServices(string ClaimID, string Slno, string IswithSI, string ClaimServiceSubType, string MemberPolicyID, string ClaimTypeID, string APIFalg, string l3ID)
        {
            try
            {
                if (Session[SessionValue.RegionID] != null)
                {
                    string message;
                    DataSet dt = new DataSet();
                    _objMadicalScrutinyVM.OPDCodingCalculationOtherServices(ClaimID, Slno, IswithSI, ClaimServiceSubType, MemberPolicyID, ClaimTypeID, APIFalg, l3ID, out message);
                    return this.Json(JsonConvert.SerializeObject(message, Newtonsoft.Json.Formatting.Indented), JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return this.Json(JsonConvert.SerializeObject("ErrorCode#1", Newtonsoft.Json.Formatting.Indented), JsonRequestBehavior.AllowGet);
        ...
        [HttpPost]
        public ActionResult ClaimProcessUnlockedByUser(long claimId)
        {
            var response = new ApiResponse<string>();
            try
            {
                ...
                {
                    var db = MvcApplication.MemoryDB;
                    long currentUserId = Convert.ToInt64(Session[SessionValue.LoginUserID]);
                    RedisValue claimValue = db.HashGet(_claimLockKey, claimId.ToString());
                    var clmInfo = JsonConvert.DeserializeObject<ClaimLockModel>(claimValue);
                    if (claimValue.HasValue)
                    {
                        // Locked by another user
                        if (clmInfo.UserId != currentUserId)
                        {
                            response.Success = false;
                            response.Message = "You are not authorized to unlock this claim.";

                            return Json(response, JsonRequestBehavior.AllowGet);
                        }
                        else if (clmInfo.UserId == currentUserId)
                        {
                            db.HashDelete(_claimLockKey, claimId.ToString());
                            response.Success = true;
                            response.Message = "Claim is unlocked.";
                            return Json(response, JsonRequestBehavior.AllowGet);
                        }
                    }
                    ...
                    db.HashDelete(_claimLockKey, claimId.ToString());
                    response.Success = true;
                    response.Message = $"Claim unlocked successfully by {clmInfo.UserName}.";
                }
                return Json(response, JsonRequestBehavior.AllowGet);
            ...
            catch (Exception ex)
            {
                ...
                response.Success = false;
                response.Message = "An error occurred while unlocking the claim.";
        ...
        public string GetBSIForAI(long ClaimID, byte SlNo = 0)
        {
            ...
            try
            {
                ...
                // SlNo and the MemberPolicyID fallback in ONE round trip, replacing two
                // separate connections (the second only opened when basicData came back
                // without a MemberPolicyID). Fetching both eagerly costs nothing extra -
                // it is the same row either way.
                long memberPolicyIdFromClaims = 0;
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    ...
                    using (var rdr = cmd.ExecuteReader())
                    {
                        if (rdr.Read())
                        {
                            if (SlNo == 0 && rdr["DetailsSlNo"] != DBNull.Value)
                                SlNo = Convert.ToByte(rdr["DetailsSlNo"]);
                            if (rdr["MemberPolicyID"] != DBNull.Value)
                                memberPolicyIdFromClaims = Convert.ToInt64(rdr["MemberPolicyID"]);
                        }
                    }
                }
                if (SlNo == 0) SlNo = 1;
                LogAI("GetBSIForAI", "resolved SlNo=" + SlNo
                    + " (MemberPolicyID from Claims=" + memberPolicyIdFromClaims + ")");
                ...
                // Fallback: MemberPolicyID straight from Claims if basicData didn't carry
                // it. Already fetched above, so no second connection.
                if (memberPolicyId == 0)
                    memberPolicyId = memberPolicyIdFromClaims;
