    ...
    public class MedicalScrutinyController : Controller
    {
                ...
                try
                {
                    ...
                    if (long.TryParse((claimId ?? "").Trim(), out claimIdForSoc))
                    {
                        ...
                        using (var connSoc = new System.Data.SqlClient.SqlConnection(connStrSoc))
                        {
                            ...
                            cmdSoc.CommandText = "SELECT TOP 1 Slno FROM Claimsdetails WHERE ClaimID=@cid AND ISNULL(Deleted,0)=0 ORDER BY Slno";
                            cmdSoc.Parameters.AddWithValue("@cid", claimIdForSoc);
                            var vSoc = cmdSoc.ExecuteScalar();
                            if (vSoc != null && vSoc != DBNull.Value) slForSoc = Convert.ToInt32(vSoc);
                        }
                    }
        ...
        private string GetClaimDiseaseTypeForStaging(long claimId, string connStr)
        {
            ...
            try
            {
                ...
                if (level3Id <= 0) return ClassifyDiseaseByDiagnosisTextForStaging(claimId, connStr);
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    var cmd = conn.CreateCommand();
                    cmd.CommandTimeout = 300;
                    cmd.CommandText = "SELECT TOP 1 Level1, Level3 FROM TPAProcedures WHERE ID=@id AND ISNULL(Deleted,0)=0";
                    cmd.Parameters.AddWithValue("@id", level3Id);
                    using (var rdr = cmd.ExecuteReader())
                    {
                        if (rdr.Read())
                        {
                ...
                // TKR: identify by the specific KNEE procedures (not the whole Orthopedics
                // specialty) — Level3 names a knee replacement / re-do knee replacement /
        ...
        public ActionResult GetClaimType(string claimId)
        {
            try
            {
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    if (string.IsNullOrEmpty(claimAiUrl))
                    {
                        aiCallError = "ClaimAIUrl not configured in Web.config";
                    }
                    else
                    {
                        try
                        {
                            // Force TLS 1.2 + bypass SSL cert validation — match what audit/start uses
                            // audit/start works fine with these settings; mirror them here.
                            System.Net.ServicePointManager.SecurityProtocol =
                                System.Net.SecurityProtocolType.Tls12 |
                                System.Net.SecurityProtocolType.Tls11 |
                                System.Net.SecurityProtocolType.Tls;
                            ...
                            using (var http = new System.Net.Http.HttpClient())
                            {
                                ...
                                var fullUrl = claimAiUrl + "/api/classify-claim-type";
                                var res = http.PostAsync(fullUrl, body).GetAwaiter().GetResult();
                                aiResponseBody = res.Content.ReadAsStringAsync().GetAwaiter().GetResult();

                                if (res.IsSuccessStatusCode)
                                {
                                    dynamic result = Newtonsoft.Json.JsonConvert.DeserializeObject(aiResponseBody);
                                    claimType = result?.claimType?.ToString() ?? "other";
                                }
                                else
                                {
                                    aiCallError = "HTTP " + (int)res.StatusCode + " from ClaimAI";
                                }
                            }
                        }
                        catch (Exception aiEx)
                        {
                            aiCallError = aiEx.GetType().Name + ": " + aiEx.Message;
                            if (aiEx.InnerException != null) aiCallError += " | inner: " + aiEx.InnerException.Message;
                        }
                    }

                    // Always log detailed result so we can debug from log file
                    try
                    {
                        string logDir = System.Web.Hosting.HostingEnvironment.MapPath("~/App_Data/Logs");
                        if (!System.IO.Directory.Exists(logDir)) System.IO.Directory.CreateDirectory(logDir);
                        System.IO.File.AppendAllText(System.IO.Path.Combine(logDir, "ClaimType_" + DateTime.Now.ToString("yyyyMMdd") + ".log"),
                            DateTime.Now.ToString("HH:mm:ss") + $" claimId={claimIdLong} url={claimAiUrl} result={claimType} error={aiCallError} responseBody={aiResponseBody}\n");
                    }
                    catch { }
        ...
        public ActionResult ProcessStagingClaims(int batchSize = 10)
        {
            ...
            try
            {
                ...
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    ...
                    var cmd = conn.CreateCommand();
                    cmd.CommandText = @"
                                    SELECT TOP (@bs) a.ClaimID AS ID
                                    FROM claimActionItems a WITH (NOLOCK)
                                    LEFT JOIN dbo.ClaimAI_Results r WITH (NOLOCK) ON r.ClaimID = a.ClaimID
                                    WHERE a.ClaimStageID = 52
                                      AND a.CloseDate IS NULL
                                      AND (r.ID IS NULL OR r.ProcessingStatus IS NULL)
                                    ORDER BY a.ClaimID DESC";
        ...
        private void SetClaimStage(string connStr, long claimId, int stageId)
        {
            using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
            {
                ...
                // ClaimAiactions table — mirror the stage onto the active (non-deleted)
                // row so the worker won't re-pick this claim on the next run.
                var cmd2 = conn.CreateCommand();
                cmd2.CommandTimeout = 300;
                cmd2.CommandText = "UPDATE claimActionItems SET ClaimStageID = @stage WHERE ClaimID = @cid AND CloseDate IS NULL";
        ...
        private long ResolveClaimDetailsRowId(string connStr, long claimId, int slNo)
        {
            try
            {
                using (var conn = new System.Data.SqlClient.SqlConnection(connStr))
                {
                    conn.Open();
                    var cmd = conn.CreateCommand();
                    cmd.CommandText = "SELECT TOP 1 ID FROM Claimsdetails WHERE ClaimID=@cid AND Slno=@sl AND ISNULL(Deleted,0)=0 ORDER BY ID DESC";
                    cmd.Parameters.AddWithValue("@cid", claimId);
                    cmd.Parameters.AddWithValue("@sl", slNo);
                    var v = cmd.ExecuteScalar();
                    if (v != null && v != DBNull.Value) return Convert.ToInt64(v);
                    // Fallback: no exact SlNo match — use the lowest SlNo row (matches how
                    // staging picks slNo: ORDER BY Slno ASC).
                    var cmd2 = conn.CreateCommand();
                    cmd2.CommandText = "SELECT TOP 1 ID FROM Claimsdetails WHERE ClaimID=@cid AND ISNULL(Deleted,0)=0 ORDER BY Slno ASC";
                    cmd2.Parameters.AddWithValue("@cid", claimId);
                    var v2 = cmd2.ExecuteScalar();
                    if (v2 != null && v2 != DBNull.Value) return Convert.ToInt64(v2);
                }
            }
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                if (reader["Diagnosis"] != DBNull.Value)
                                    diagnosis = reader["Diagnosis"].ToString().Trim();
                                if (reader["PresentComplaint"] != DBNull.Value)
                                    presentComplaint = reader["PresentComplaint"].ToString().Trim();
                                if (reader["ExecutiveNotes"] != DBNull.Value)
                                    processingRemarks = reader["ExecutiveNotes"].ToString().Trim();
        ...
        public ActionResult IsClaimAISummaryAllowed(string claimId = null, string slNo = null)
        {
            try
            {
                ...
                // Cashless (Preauth):        ClaimTypeID = 1, RequestTypeID = 1
                // Reimbursement (Final Bill): ClaimTypeID = 2, RequestTypeID = 4
                // Both are processed by the AI engine with the same rules for now.
                bool allowed = (claimTypeId == 1 && requestTypeId == 1) || (claimTypeId == 2 && requestTypeId == 4);
                ...
                return Json(new
                {
                    ...
                    reason = allowed ? "OK" : "AI Summary is only available for Cashless (Preauth) and Reimbursement (Final Bill) claims"
                }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
